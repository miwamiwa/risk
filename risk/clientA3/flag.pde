...
void makeFlag(){
  ...
  {
   switch(key){
    case 'z': colorSelection = clientId*3; break;
    case 'x': colorSelection = clientId*3+1; break;
    case 'c': colorSelection = clientId*3+2; break;
...
void newFlagPixel(int index){
  ...
  fill(flagColors[colorSelection]);
  rect( 
     flagX + j*flagGridSize,
     flagY + i*flagGridSize,
     flagGridSize,
...
void sendMap(){
  ...
  println("sendmap");
  if(clientId!=-1){
   for(int i=0; i<playerTiles[clientId].size(); i++){
 //    println("setit");
     setTileColor(clientId,playerTiles[clientId].get(i));
   }
  }
